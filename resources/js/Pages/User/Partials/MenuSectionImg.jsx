...
export default function MenuSectionImg({ section, deliteMenuSection, updateMenu, frontErrors }) {
    ...
    return (
        <div className="p-3 w-full border border-2 border-gray-500 mt-2 relative text-black">
            ...
            <div className="w-64  self-center md:self-start mb-3">
                <h1>
                    <input className={"p-2 rounded-sm " + styling.input[theme]} type="text" placeholder={locale == 'en' ? "Set section Title" : "Unesite naslov sekcije"} value={menuSectionModal.title || ''} onChange={e => setMenuSectionModal({ ...menuSectionModal, title: e.target.value })} />
                </h1>
                {
                ...
                <div className="flex justify-start gap-2 ">
                    {
                        menuSectionModal.items.map((item, ind) => (
                            <Item key={item.id} item={item} deleteSectionItem={deleteSectionItem} updateSection={updateSection} frontErrors={frontErrors} />
                        ))
                    }
